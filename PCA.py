...
def GetMeanImages(images, labels=None):
...
def GetPCImage(V, idx, ImageSize):
...
def SaveImage(image, FigurePath):
...
def AverageImages(SD, images, labels):
    ...
    OverallMeanImage = GetMeanImages(images)

    print(f'\tSaving image to: \"{OutputDir}\"...')
    SaveImage(OverallMeanImage, os.path.join(OutputDir, f"mean-overall.png"))

    print('Calculating label-wise mean images...')
    MeanImages, MeanLabels = GetMeanImages(images, labels)
    ...
    for img, lbl in zip(MeanImages, MeanLabels):
        SaveImage(img, os.path.join(OutputDir, f"mean-{lbl}"))
...
for component in range(100):
    SavePCImage(V, component, os.path.join(ComponentImagePath, f'Component-{component+1}.png'), SD.ImageSize)

print(f'Saving PCA components to data file...')
PCAComponents = np.array([GetPCImage(V, component, (-1, 1)) for component in range(V.shape[0])])
np.save(f'{SD.name}-Component-V.npy', PCAComponents)
